...
export const ButtonOfCart = memo(function ButtonOfCart({
  ...
  const containerClasses = useMemo(
    () =>
      `absolute flex items-center justify-end rounded-md right-1 overflow-hidden z-1 ${
    ...
    <motion.div className={containerClasses} transition={transition}>
            ...
            >
              <AnimatePresence mode="wait" initial={false}>
                {isLastItem ? (
                  ...
                  >
                    <FaRegTrashCan aria-hidden="true" className="size-4" />
                  </motion.span>
                ) : (
                  <motion.span
                    key="minus"
      ...
      >
        {!slideOpen && productCant > 0 ? (
          ...
          <TbShoppingCartPlus className="size-4" aria-hidden="true" />
        )}
      </button>
    </motion.div>
  );

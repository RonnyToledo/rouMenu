...
export default function PostCard({ filterIndex }: { filterIndex: number }) {
  ...
  return generalData.top_posts
    ...
    .map((obj, index) => (
      <div
        key={index}
        className="mb-4 p-4 space-y-2 shadow-lg bg-white dark:bg-slate-900 rounded-lg"
      >
        <Link
          href={`/t/${obj.store_sitioweb}`}
          className="flex items-center gap-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
        >
          <Image
            ...
            width={100}
            height={100}
            className="rounded-full size-10 bg-center bg-no-repeat aspect-square bg-cover ring-2 ring-slate-200 dark:ring-slate-600"
          />
        ...
        <div className="">
          <Link
            ...
            className="flex flex-col items-stretch justify-start rounded-lg w-full space-y-2 group"
          >
            <Image
              ...
              width={500}
              height={500}
              className="w-full object-cover aspect-square bg-cover rounded-lg group-hover:opacity-90 transition-opacity"
            />
            <div className="flex w-full grow flex-col items-stretch justify-center gap-1 @xl:px-4">
              <p className="text-[#0d141c] dark:text-slate-100 text-lg font-bold leading-tight tracking-[-0.015em]">
                {obj.title}
              </p>
              <div className="flex items-end gap-3 justify-between">
                <div className="flex flex-col gap-1 w-full">
                  ...
                  <div className="flex justify-evenly items-center my-4">
                    <div className="flex text-slate-600 dark:text-slate-400 items-center gap-1">
